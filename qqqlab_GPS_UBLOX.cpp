...
void AP_GPS_UBLOX::_verify_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate) {
  ...
  if (_class == CLASS_CFG) {
    switch (_msg_id) {
      case MSG_CFG_NAV_SETTINGS:
        ...
        if (_navfilter != GPS_ENGINE_NONE && _buffer.nav_settings.dynModel != _navfilter) {
        ...
        if (_min_elevation != -100 && _buffer.nav_settings.minElev != _min_elevation) {
      ...
      case MSG_CFG_PRT:
        ...
        return false;
      case MSG_CFG_RATE:
        if (_buffer.nav_rate.measure_rate_ms != rate_ms || _buffer.nav_rate.nav_rate != 1 || _buffer.nav_rate.timeref != 0) {
          _configure_rate();
      ...
      case MSG_CFG_TP5:
        {
          ...
          if (_buffer.nav_tp5.flags != desired_flags || _buffer.nav_tp5.freqPeriod != desired_period_hz) {
      ...
      case MSG_CFG_VALGET:
        {
          ...
          while (cfg_len >= 5) {
            ...
            if (cfg_idx >= 0) {
              ...
              if (active_config.fetch_index >= 0 && active_config.fetch_index < active_config.count && id == active_config.list[active_config.fetch_index].key) {
                active_config.fetch_index++;
                if (active_config.fetch_index < active_config.count) {
                  _configure_valget(active_config.list[active_config.fetch_index].key);
      ...
      case MSG_MON_VER:
        {
          ...
          interface_printf("ublox-%s %s%s HW:%s SW:%s\n", gen, _module, mod != nullptr ? " " : "", _version.hwVersion, _version.swVersion);
  ...
  switch (_msg_id) {
    ...
    case MSG_STATUS:
      ...
      if (_buffer.status.fix_status & NAV_STATUS_FIX_VALID) {
        if ((_buffer.status.fix_type == AP_GPS_UBLOX::FIX_3D) && (_buffer.status.fix_status & AP_GPS_UBLOX::NAV_STATUS_DGPS_USED)) {
    ...
    case MSG_SOL:
      ...
      if (_buffer.solution.fix_status & NAV_STATUS_FIX_VALID) {
        if ((_buffer.solution.fix_type == AP_GPS_UBLOX::FIX_3D) && (_buffer.solution.fix_status & AP_GPS_UBLOX::NAV_STATUS_DGPS_USED)) {
          next_fix = GPS_OK_FIX_3D_DGPS;
    ...
    case MSG_PVT:
      ...
      state.year = _buffer.pvt.year;
      state.month = _buffer.pvt.month;
      state.day = _buffer.pvt.day;
      state.hour = _buffer.pvt.hour;
      state.min = _buffer.pvt.min;
      state.sec = _buffer.pvt.sec;
      state.valid = _buffer.pvt.valid;
 ...
 */
#ifdef HAL_GPIO_PPS
void AP_GPS_UBLOX::pps_interrupt(uint8_t pin, bool high, uint32_t timestamp_us) {
  _last_pps_time_us = AP_HAL::micros64();
}

void AP_GPS_UBLOX::set_pps_desired_freq(uint8_t freq) {
  _pps_freq = freq;
  _unconfigured_messages |= CONFIG_TP5;
}
#endif
 ...
 */
void AP_GPS_UBLOX::_update_checksum(uint8_t *data, uint16_t len, uint8_t &ck_a, uint8_t &ck_b) {
  while (len--) {
    ck_a += *data;
    ck_b += ck_a;
    data++;
 ...
 */
bool AP_GPS_UBLOX::_send_message(uint8_t msg_class, uint8_t msg_id, const void *msg, uint16_t size) {
 ...
 */
bool AP_GPS_UBLOX::_request_message_rate(uint8_t msg_class, uint8_t msg_id) {
 ...
 */
bool AP_GPS_UBLOX::_configure_message_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate) {
 ...
 */
bool AP_GPS_UBLOX::_configure_valset(ConfigKey key, const void *value, uint8_t layers) {
  if (!supports_F9_config()) {
    return false;
  }
 ...
 */
bool AP_GPS_UBLOX::_configure_valget(ConfigKey key) {
  if (!supports_F9_config()) {
    return false;
  }
  struct {
 ...
 */
bool AP_GPS_UBLOX::_configure_config_set(const config_list *list, uint8_t count, uint32_t unconfig_bit, uint8_t layers) {
 ...
 */
bool AP_GPS_UBLOX::_configure_list_valset(const config_list *list, uint8_t count, uint8_t layers) {
  if (!supports_F9_config()) {
    return false;
  }
...
/*
  ...
  matches a UBlox
 */
bool AP_GPS_UBLOX::_detect(struct UBLOX_detect_state &state, uint8_t data) {
reset:
  switch (state.step) {
    case 0:
      if (PREAMBLE1 == data) {
...
void AP_GPS_UBLOX::broadcast_configuration_failure_reason(void) {
